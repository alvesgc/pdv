    ...
    <div className="min-h-screen bg-gray-100 p-4 font-sans text-gray-800">
      <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="md:col-span-1 bg-white p-6 rounded-lg shadow-lg flex flex-col">
          <h2 className="text-2xl font-bold text-center text-gray-900 mb-6">Carrinho de Compras</h2>
          <div className="flex-grow overflow-y-auto max-h-96 pr-2 custom-scrollbar"> {/* Adicionado max-h e scrollbar */}
            {cart.length === 0 ? (
              <p className="text-center text-gray-500">Carrinho vazio. Adicione produtos!</p>
            ) : (
              cart.map((p, i) => (
                <div
                  key={i}
                ...
                >
                  <span className="text-gray-700">{p.name} - R$ {p.price.toFixed(2)}</span>
                  <button
          ...
          <div className="mt-6 pt-4 border-t-2 border-gray-200">
            <h3 className="text-3xl font-bold text-right text-gray-900 mb-4">Total: <span className="text-blue-600">R$ {total.toFixed(2)}</span></h3>
            <button
              onClick={clearCart}
              disabled={cart.length === 0}
              className={`w-full py-3 px-4 rounded-md text-white font-semibold transition duration-150 ease-in-out ${
                cart.length === 0 ? 'bg-gray-400 cursor-not-allowed' : 'bg-green-600 hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-opacity-50'
              }`}
            >
              Finalizar Venda
            </button>
